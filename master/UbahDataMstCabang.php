...
$UbahDataMstCabang = Query("SELECT mst_cabang.kode_cabang, mst_cabang.nama_cabang, mst_cabang.alamat, mst_propinsi.kode_propinsi, mst_propinsi.nama_propinsi, mst_kabupaten_kota.kode_kabupaten_kota, mst_kabupaten_kota.nama_kabupaten_kota, mst_cabang.no_telepon
	FROM mst_cabang
	INNER JOIN mst_propinsi ON mst_cabang.kode_propinsi = mst_propinsi.kode_propinsi
	INNER JOIN mst_kabupaten_kota ON mst_cabang.kode_kabupaten_kota = mst_kabupaten_kota.kode_kabupaten_kota
	WHERE kode_cabang = '$KodeCabang'")[0];

if (isset($_POST["TombolUbah"])) {
	if (UbahDataMstCabang($_POST) > 0) {
		echo "
			<script>
...
<div class="container">
	<form action="" method="POST" id="FormEntri">
		<div class="form-group row">
			<label for="KodeCabang" class="col-sm-3 col-form-label">Kode Cabang</label>
			<div class="col-sm-4">
				<input type="text" class="form-control" id="KodeCabang" name="kode_cabang" value="<?php echo $UbahDataMstCabang["kode_cabang"]; ?>" readonly/>
			</div>
		</div>
		<div class="form-group row">
			<label for="NamaCabang" class="col-sm-3 col-form-label">Nama Cabang</label>
			<div class="col-sm-4">
				<input type="text" class="form-control" id="NamaCabang" name="nama_cabang" placeholder="Masukan Nama Cabang" autocomplete="off" maxlength="50" required oninvalid="this.setCustomValidity('Nama Cabang Harus Diisi.')" oninput="setCustomValidity('')" value="<?php echo $UbahDataMstCabang["nama_cabang"]; ?>"/>
			</div>
		</div>
		<div class="form-group row">
			<label for="Alamat" class="col-sm-3 col-form-label">Alamat</label>
			<div class="col-sm-4">
				<textarea class="form-control" name="alamat" id="Alamat" cols="50" rows="4" maxlength="150" required oninvalid="this.setCustomValidity('Alamat Harus Diisi.')" oninput="setCustomValidity('')"><?php echo $UbahDataMstCabang["alamat"]; ?></textarea>
			</div>
		</div>
		<div class="form-group row">
			<label for="NamaPropinsi" class="col-sm-3 col-form-label">Nama Propinsi</label>
			<div class="col-sm-3">
				<input type="hidden" class="form-control" id="KodePropinsi" name="kode_propinsi" value="<?php echo $UbahDataMstCabang["kode_propinsi"]; ?>"/>
				<input type="text" class="form-control" id="NamaPropinsi" name="nama_propinsi" placeholder="Nama Propinsi" value="<?php echo $UbahDataMstCabang["nama_propinsi"]; ?>" readonly/>
			</div>
			<div class="col-sm-1">
		...
		<div class="form-group row">
			...
			<div class="col-sm-3">
				<input type="hidden" class="form-control" id="KodeKabupatenKota" name="kode_kabupaten_kota" value="<?php echo $UbahDataMstCabang["kode_kabupaten_kota"]; ?>"/>
				<input type="text" class="form-control" id="NamaKabupatenKota" name="nama_kabupaten_kota" placeholder="Nama Kabupaten / Kota" value="<?php echo $UbahDataMstCabang["nama_kabupaten_kota"]; ?>" readonly/>
			</div>
			<div class="col-sm-1">
		...
		<div class="form-group row">
			...
			<div class="col-sm-4">
				<input type="tel" class="form-control" id="NomorTelepon" name="no_telepon" autocomplete="off" placeholder="Masukan Nomor Telepon" pattern="^\d{0,13}$" title="Nomor Telepon Harus Menggunakan Angka & Tidak Boleh Lebih Dari 13 Digit." required oninvalid="this.setCustomValidity('Nomor Telepon Harus Diisi.')" oninput="setCustomValidity('')" value="<?php echo $UbahDataMstCabang["no_telepon"]; ?>"/>
			</div>
		</div>
		<div class="form-group row">
			...
			<div class="col-sm-4">
				...
				<a href="../master/FormDataMstCabang.php"><button type="button" id="TombolBatal" name="TombolBatal" class="btn btn-outline-secondary">Batal</button></a>
			</div>
		</div>
	</form>
